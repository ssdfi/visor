...
export var map = new L.map('mapid', {
    layers : baseTreeMap[0]["children"][0].layer
}).
setView([init_lat, init_long], 5);
//desde acá seteo que capaz quiero q aparezcan prendidas por default
try{
  //macizos
  let default_layer = baseLayersTree[2].children[0].children[1].layer;
  //agentes regionales
  let default_layer_2 = baseLayersTree[2].children[0].children[0].layer;
  //cortinas
  let default_layer_3 = baseLayersTree[2].children[0].children[4].layer;
  
  console.log(baseLayersTree[2].children[0].children[0].layer)
  default_layer.addTo(map);
  default_layer_2.addTo(map);
  default_layer_3.addTo(map);
} catch(error) {
  console.log(error)
}
